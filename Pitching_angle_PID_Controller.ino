...
double xangle, yangle, zangle;// for storing x, y,
z angles

int chipSelect = 10; //chipSelect pin for the SD
card Reader
File angleFile; //Data object you will write your
sesnor data to
unsigned long myTime;

double Output_max=20;//30 // servo maximum rotation
from 90
double Output_min=-20;//30 //servo minimum rotation
from 90
...
AutoPID
pitchPID(&pitch,&setpoint,&pidXout,Output_min,Output
_max, Kp, Ki, Kd);// autopid_pitch

Servo Xservo;// pitch servo
int servoAngleDefault = 85 ; //Default servoAngle
i.e. 0th posn of TVC
...
while(status!=0){ } // stop everything if could
not connect to MPU6050
Serial.println(F("Calculating offsets, do not
move MPU6050"));
// mpu.upsideDownMounting = true; //
uncomment this line if the MPU6050 is mounted
upside-down
...
Xservo.write(0);
Serial.println("Servo Angle =0"); // To display on
screen
...
Xservo.write(125);
Serial.println("Servo Angle =125"); // To display
on screen
delay(10000); //10s delay
Xservo.write(85);// Flag Servo signal at 90 deg
(physical) =85 deg(electrical)
Serial.println("Servo Angle =85"); // To display
on screen
delay(2000);
pitchPID.setTimeStep(40);//set PID update interval
to 40ms

}

void loop()
{Serial.println("Loop started"); // To display on
screen
mpu.update();

myTime = millis();

//xangle= mpu.getAngleX(); // Read xangle from
MPU6050
//yangle = mpu.getAngleY(); // Yangle = Pitch
angle
//zangle= mpu.getAngleZ(); //zangle
angleFile = SD.open("pitchAng.txt",
FILE_WRITE); // File saved as SAMPLE.txt
if (angleFile)
{
...
pidXout=servoAngleDefault+pidXout;//offset of
pitch servo 85
